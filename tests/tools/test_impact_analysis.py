...
def _caller_src(ref_id: str) -> str:
    return f'void Run() {{ pfmDbioSelect("{ref_id}"); }}'


def test_find_callers_confirms_true_positive_and_drops_false_positive():
    ...
    result = service.find_callers("dbio", dbio_id, searcher, reader)
...
def test_find_callers_classifies_by_module_type():
    dbio_id = "PFO_STCK_MA_DS200"
...
def test_find_callers_biz_ref_type_confirms_via_biz_call_macro():
    # DBIO뿐 아니라 Biz ID도 같은 파이프라인으로 확정된다 — pfmDlCall("...")로 호출하는
    # 다른 Biz를 찾는 재귀 펼치기 시나리오.
    biz_id = "MPCOM_GetBzopDate"
    caller_path = module_path("biz", "PCOM", "MPCOM_Caller")
    other_path = module_path("service", "PCOM", "SVC_Unrelated")
    searcher = FakeSearcher([caller_path, other_path])
    reader = FakeReader({
        caller_path: f'void Run() {{ pfmDlCall("{biz_id}", "{biz_id}", &in, &out); }}',
        other_path: f'void Run() {{ pfmDbioSelect("{biz_id}"); }}',  # dbio 콜 매크로라 biz로 확정 안 됨
    })

    result = service.find_callers("biz", biz_id, searcher, reader)

    assert result.bizs == ["MPCOM_Caller"]
    assert result.services == []


def test_find_callers_skips_path_not_matching_module_convention():
    searcher = FakeSearcher(["/some/unrelated/path.c"])
    reader = FakeReader({})

    result = service.find_callers("dbio", "PFO_STCK_MA_DS200", searcher, reader)

    assert result.bizs == result.services == result.batches == []
    assert reader.read_calls == []


def test_find_callers_skips_candidate_on_source_not_found():
...
def test_find_callers_propagates_source_error():
    path = module_path("biz", "PCOM", "MPCOM_Err")
    searcher = FakeSearcher([path])
    reader = FakeReader(raise_error_for={path})

    with pytest.raises(SourceError):
        service.find_callers("dbio", "PFO_STCK_MA_DS200", searcher, reader)


def test_find_callers_empty_candidates_returns_empty_without_reading():
    searcher = FakeSearcher([])
    reader = FakeReader({})

    result = service.find_callers("dbio", "NO_SUCH_DBIO", searcher, reader)

    assert result.services == result.bizs == result.batches == []
    assert reader.read_calls == []


def test_find_callers_scopes_grep_to_resource_groups():
    dbio_id = "PFO_STCK_MA_DS200"
    searcher = FakeSearcher([])
    reader = FakeReader({})

    service.find_callers("dbio", dbio_id, searcher, reader, resource_groups=["PCOM", "NCOM"])
...
def test_router_callers_confirms_and_classifies():
    ...
    resp = client.get(f"/impact-analysis/callers/dbio/{dbio_id}")

    assert resp.status_code == 200
    assert resp.json() == {
        "ref_type": "dbio", "ref_id": dbio_id,
        "services": [], "bizs": ["MPCOM_Biz"], "batches": [],
    }


def test_router_callers_biz_ref_type_works():
    biz_id = "MPCOM_GetBzopDate"
    caller_path = module_path("service", "PCOM", "SVC_Caller")
    _use(
        FakeSearcher([caller_path]),
        FakeReader({caller_path: f'void Run() {{ pfmDlCall("{biz_id}", "{biz_id}", &in, &out); }}'}),
    )

    resp = client.get(f"/impact-analysis/callers/biz/{biz_id}")

    assert resp.status_code == 200
    assert resp.json() == {
        "ref_type": "biz", "ref_id": biz_id,
        "services": ["SVC_Caller"], "bizs": [], "batches": [],
    }


def test_router_callers_invalid_ref_type_rejected():
    # service/batch는 "누가 부르나"를 물을 대상이 아님(CallerRefType은 dbio/biz만 허용).
    resp = client.get("/impact-analysis/callers/service/SVC_Something")
    assert resp.status_code == 422


def test_router_callers_empty_ref_id_rejected():
    resp = client.get("/impact-analysis/callers/dbio/%20")  # strip 후 빈 문자열
    assert resp.status_code == 400


def test_router_callers_grep_failure_returns_503():
    _use(FakeSearcher([], raise_error=True), FakeReader({}))
    resp = client.get("/impact-analysis/callers/dbio/PFO_STCK_MA_DS200")
    assert resp.status_code == 503
...
def test_router_callers_passes_resource_groups_query_param():
    ...
    resp = client.get(
        f"/impact-analysis/callers/dbio/{dbio_id}",
