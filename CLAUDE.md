...
| **Table Extractor** | `table_extractor` | id_type/업무그룹/ID → 원격지(SFTP) 소스 읽기 → 참조 테이블 추출 | **DBIO 구현됨**, Service/Batch/Biz 준비 중 |
...
(cd remote_server && docker compose up -d)   # 정지: docker compose down
...
- `proframe.py`의 `IdType` / `Prog`(`Literal` 타입): ProFrame ID 분류 체계(`dbio/service/batch/biz`, 업무그룹 `PCSP/PCSH/NCOM/NCSP/PCOM/PPFR/RLGR`). id_type 4종 전체에 걸친 값이라 공용에 둠 — `table_extractor`가 먼저 쓰지만 다른 툴(예: Migration Builder)도 소비처가 될 예정.
- `dbio.py`의 `read_dbio_xml(prog, id, reader) -> str` / `classify_sqltype(id) -> str` / `UnknownSqlType`: DBIO ID로 원격 XML을 읽는 범용 동작. ID 끝 2글자 코드(`DBIO_RESOURCE_ROOT`/`SQL_TYPE_BY_SUFFIX`/`ID_SUFFIX_RE`로 표현)가 SQLTYPE 디렉토리를 확정하므로 후보 디렉토리 순회 없이 경로 1회 조합·조회로 끝난다. "DBIO ID → 원격 XML"은 table_extractor만이 아니라 소비처가 늘어날 동작이라 공용화(단순 값이 아니라 로직까지 포함 — `proframe.py`와 달리 이 모듈은 동작 단위로 공용).
- `source.py`의 `SourceReader`(Protocol) / `SftpSourceReader` / `SourceNotFound`·`SourceError` / `default_reader()`: 원격 소스 I/O 인프라(경로→내용 문자열). 회사 서버가 SFTP라 paramiko 기반. `default_reader()`는 env(`NOGADA_SFTP_HOST/_PORT/_USER/_PASS/_BASE`, 기본값 `remote_server` Docker SFTP 127.0.0.1:2222)에서 `SftpSourceReader`를 조립하는 팩토리 — 회사 SFTP 서버는 하나뿐이라 여러 툴이 공유. 라우터에 `Depends(default_reader)`로 주입하면 테스트에서 `app.dependency_overrides`로 인메모리 fake reader 교체 가능.
...
`sql_bench`가 SQL 텍스트를 직접 받는 반면, `table_extractor`는 **식별자(id_type/prog/id)로 원격 파일을 찾아 읽은 뒤** 그 안의 SQL을 `extract_tables`(공용)로 넘긴다. REST 계약은 부작용 없는 조회라 **`GET /table-extractor/{id_type}/{prog}/{id}`**(바디 없음, 경로 파라미터). `id_type`/`prog`는 `Literal`(공용 `proframe.py`)이라 잘못된 값은 FastAPI가 422로 자동 거부.

흐름: `router.py`(`Depends(default_reader)`로 SFTP reader 주입) → `service.extract(id_type, prog, id, reader)`:
1. 지금은 `id_type == "dbio"`만 처리(그 외는 라우터가 501). Service/Batch/Biz는 상위 `.c` 소스에서 하위 ID를 스캔·재귀 해석하는 후속 작업(`plan.md` 참고).
2. `dbio.read_dbio_xml`(공용): ID 끝 2글자 코드로 SQLTYPE을 확정(`classify_sqltype`) → `<PROG>/<SQLTYPE>/<ID>/<ID>.xml` 경로 1회 조합 → `reader.read()`. 인식 못 하는 접미사는 `UnknownSqlType`(→라우터 400).
...
에러 매핑(라우터): `UnknownSqlType`/`ExtractionError`→400, `SourceNotFound`→404, `id_type != dbio`→501. (`SourceError` 접속실패는 아직 매핑 안 돼 500 — 후속 502/503 예정.)
...
프론트(`table_extractor.js`): `#te-submit` 클릭 → 경로 조합해 `fetch` → **좌측 패널** = 테이블 목록(실시간 텍스트 필터 + `TRU/PFO/PTN/RPT` 접두사 필터 + 개별 삭제 + 전체/개별 복사, `FEP` 접두사는 이관 미지원이라 하단에 읽기 전용 분리). **우측 패널** = PK 키 입력 + 이관 SQL 생성(위 `/pks`·`/migrate-sql` 항목 참고). 업무그룹 입력은 7개 값(`Prog`)을 필터링하는 검색형 콤보박스.
...
app/
  ...
  common/proframe.py          # IdType/Prog (ProFrame ID 분류 체계, 툴 공용)
